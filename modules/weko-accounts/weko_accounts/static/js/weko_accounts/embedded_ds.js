...
let wayf_width = "390";
let wayf_background_color = '#F8F8FF';
let wayf_font_color = '#000080';
let wayf_border_color = '#214890';
let wayf_font_size = 12;
let wayf_hide_logo = false;
let wayf_logged_in_messsage = "You are already authenticated.";
let wayf_hide_after_login = false;
let wayf_most_used_idps = [];
let wayf_show_categories = true;
let wayf_hide_categories = [];
let wayf_hide_idps = [];
let wayf_unhide_idps = [];
let wayf_show_remember_checkbox = true;
...
let favorite_list = [];
let hint_list = [];
let submit_check_list = [];
let safekind = '2';
// let allIdPList = '';
let initdisp = 'Select the Home Organisation you are affiliated with';
let dispDefault = '';
let dispidp = '';
let hiddenKeyText = '';
...
//            writeHTML('<img src="https://test-ds.gakunin.nii.ac.jp/GakuNinDS/images/alert.gif" id="icon-alert" title="Alert SP!" style="vertical-align:text-bottom; border:0px; width:20px; height:20px;">');
    ...
    if (wayf_use_discovery_service == true) {
      let return_url = wayf_sp_samlDSURL + (wayf_sp_samlDSURL.indexOf('?') >= 0 ? '&' : '?') + 'SAMLDS=1&target=' + encodeURIComponent(wayf_return_url);
    ...
    if (wayf_most_used_idps.length > 0) {
      ...
      for (let i = 0; i < wayf_most_used_idps.length; i++) {
    ...
    if (wayf_additional_idps.length > 0) {
      let listcnt = inc_search_list.length;

      // Show additional IdPs in the order they are defined
      for (let i = 0; i < wayf_additional_idps.length; i++) {
    ...
    writeHTML('<td>');
    writeHTML('<img id="dropdown_img" src="' + dropdown_down + '" title="Display/non-display of IdP list" tabindex=6 style="border:0px; width:20px; height:20px; vertical-align:middle;">');
    writeHTML('</td>');
